...
//+,-로 split을 할까?
//그 다음에 /또는*가 있는 것끼리 남을테니까,새롭게 해서, 스택에넣는거야.

//* / 에 대해서 한번 싹 돌고, 그다음에 - , +에 대해 돌아보자.

//A*B*C
//ABC
//** 이렇게나옴

//*가 나오면 앞의 두개를 뽑아(A,B) AB*라고 문자열을 새로 만들고, 뒤에 다시넣음
//*, C(AB*)
//또 *가 나왔음.
//ABC**이렇게 해야 맞음.
...
//만약 )가 나온다면, (가 나올때까지 뺀다.
//자기보다 우선순위가 높은게 stack의 top에 있다면 스택에 있는 거 다 비우냐?
//
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<line.length(); i++){
            ...
            if(unit>='A'&&unit<='Z'){
                ...
                stack.offerLast(unit);
            }

            //연산자가 나왔을때, stack에 넣는다.
            //이때, 우선순위를 확인하는데,
            //연산자가 다 비어있지 않을경우, ~
            //(일때, 처리를 해줘야될듯
            else if(unit == ')'){
                ...
                while(!stack.isEmpty()&&getPriority(unit)<=getPriority(stack.peekLast())){//
                    sb.append(stack.pollLast());
                }
                stack.offerLast(unit);
            }
