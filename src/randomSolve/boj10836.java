...
//더하기(+) 기호는 앞으로 생략한다. 구체적으로 각 애벌레가 자라는 정도를 결정하는 규칙은 다음과 같다.
//제일 왼쪽 열과, 제일 위쪽 행의 애벌레들은 자신이 자라는 정도를 스스로 결정한다. 이들은 입력으로 주어질 것이다.
//애벌레들이 자라는 정도를 왼쪽 제일 아래 칸에서 시작하여 위쪽으로 가면서 읽고,
//제일 위쪽 칸에 도착하면 오른쪽으로 가면서 행의 끝까지 읽었다고 하자. 모든 입력에서 이렇게 읽은 값들은 감소하지 않는 형태이다.
...
public class boj10836 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int count = 0;
        for(int j=M-1; j>=0;j--){
            increase[j][0]=init[count++];
        }
        for(int j=1; j<M; j++){
            increase[0][j]=init[count++];
        }

        for(int j=1; j<M; j++){
            for(int k=1; k<M; k++){
                increase[j][k]= Math.max(Math.max(increase[j-1][k-1],increase[j-1][k]),increase[j][k-1]);
            }
        }

        for(int j=0; j<M; j++){
            for(int k=0; k<M; k++){
                arr[j][k]=increase[j][k]+1;
            }
        }
