...
//풀이
//"solveByTopic.bfs"에서 시간은 이전 큐에 넣은 시간+1을 의미한다.
...
//solveByTopic.bfs(graph,N)
//int solveByTopic.queue 선언
//queue에 N 넣기

//while(!solveByTopic.queue.isEmpty)
//  int x =solveByTopic.queue.poll
//  if(x<K)
//
//      if(graph[x+1]==-2) graph[x]+1=graph[x+1],break
//      if(graph[x*2]==-2) graph[x}+1=graph[x*2],break
//      if(graph[x+1]!=-1 continue
//      solveByTopic.queue.offer(x+1)
//      graph[x]+1=graph[x+1]
//
//      if(x*2>100000) continue
//      if(graph[x*2]!=-1) continue
//      solveByTopic.queue.offer(x*2)
//      graph[x]+1=graph[2*x]
//
//  if(x>0)
